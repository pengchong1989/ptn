...
public class BusinessidService_MB extends ObjectService_Mybatis {
	...
	public Businessid select(int siteId, String type, int businessId) {
		...
		try {
			...
			List<Businessid> idList = this.mapper.query(businessid);
			if(idList != null && idList.size() > 0){
				businessid = idList.get(0);
			}
		} catch (Exception e) {
	...
	public Businessid select(int siteId,String type) throws Exception{
		Businessid businessid=null;
		try {
			businessid=this.mapper.queryUseBySiteIDandType(siteId, type,0);
		} catch (Exception e) {
			ExceptionManage.dispose(e,this.getClass());
		}
		return businessid;
	...
	* @Exception 异常对象
	...
	public static void main(String[] args) {
		try {
			...
			integers.add(1);
			integers.add(2);
//			int id = businessidServiceMB.select_site(integers, "ring");
			Businessid businessid = new Businessid();
			businessid.setSiteId(2);
			businessid.setType("Tunnel");
			businessid.setIdValue(22);
			businessid.setIdValue(1);
			List<Businessid> businessids = new ArrayList<Businessid>();
	...
	public Businessid getIdValue(int siteId, String type) {
		Businessid businessid = null;
		try {
			List<Businessid> idList = this.mapper.getIdValue(siteId, type);
			if(idList != null && idList.size() > 0){
				businessid = idList.get(0);
			}
		} catch (Exception e) {
