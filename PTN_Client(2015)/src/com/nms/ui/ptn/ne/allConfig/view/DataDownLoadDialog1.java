 ...
 *
 */
public class DataDownLoadDialog1 extends PtnDialog{
	 ...
	 * @param modal
	 */
	public DataDownLoadDialog1(SiteInst siteInst, boolean modal,String action) {
		this.setModal(modal);
	...
	private boolean chech(){
		if (basicFunctions.isSelected()==false) {
			if("synchro".equals(this.action)){
				DialogBoxUtil.errorDialog(this, ResourceUtil.srcStr(StringKeysTip.TIP_SELECT_DATA_SYNCHRO));
			}else{
				DialogBoxUtil.errorDialog(this, ResourceUtil.srcStr(StringKeysTip.TIP_SELECT_DATA_DOWNLOAD));
			}
			return false;
		}else{
			return true;
	...
	private void saveActionPerformed(java.awt.event.ActionEvent evt) {
		...
		try{
			...
			if("dataDownLoad".equals(this.action)){
				siteDispatch.dataDownLoadActionPerformed(fieldConfigRightPanel.getTable().getAllSelect(),modelSelected);
				//添加日志记录
			...
			siteInstList.add(siteInst);
				this.synchro(siteInstList,modelSelected);
				//添加日志记录
				this.btnSave.setOperateKey(EOperationLogType.SITELISTSYNCHRO.getValue());
				this.btnSave.setResult(1);
				this.insertOpeLog(EOperationLogType.SITELISTSYNCHRO.getValue(), ResultString.CONFIG_SUCCESS, null, siteInst);	
			}
			this.dispose();
		} catch (Exception e) {
			ExceptionManage.dispose(e,this.getClass());
	...
	private void insertOpeLog(int operationType, String result, Object oldSeg, SiteInst newSeg){
		int siteId=newSeg.getSite_Inst_Id();
		newSeg=null;
		AddOperateLog.insertOperLog(btnSave, operationType, result, oldSeg, newSeg, siteId,ResourceUtil.srcStr(StringKeysLbl.LBL_NE_SYNC), null);	
	}
	/**
	 * 网元同步
	 * @param siteInstList
...
//						cesDispatch.synchro(siteInst.getSite_Inst_Id()); // 同步ces
				...
				if(modelSelected[7]==1){
					List<Integer> siteIds = new ArrayList<Integer>(); // 同步告警
					siteIds.add(siteInst.getSite_Inst_Id());
					alarmDispatch.executeQueryHisAlarmBySites(siteIds);
					//网元同步后，刷新告警
