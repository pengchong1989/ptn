...
import com.nms.ui.ptn.business.testoam.controller.TunnelOamBusinessController;

public class TunnelOamBusinessDialog extends PtnPanel {

	private TunnelOamBusinessController controller;
	...
	public TunnelOamBusinessDialog(Tunnel tunnel, OamInfo oamInfo_temp, TestOamMainDialog mainDialog) {
		initComponent();
		setComponentLayout();
		addListener();
		intalCombox();
		...
		controller = new TunnelOamBusinessController(this);
		this.oam_temp = oamInfo_temp;
		//lspName赋值
		this.initLspNameCombox(tunnel, oamInfo_temp);
		this.getTestOam(tunnel, oamInfo_temp);
		if (oamInfo != null) {
	...
	private void initLspNameCombox(Tunnel t, OamInfo oam) {
		DefaultComboBoxModel boxModel = null;
		try {
			lspNameComboBox.removeAllItems();
			boxModel = (DefaultComboBoxModel) lspNameComboBox.getModel();
			//把主动OAM的MEG等级,本端维护点Id,远端维护点Id赋给按需OAM对应的属性
			this.melField.setText(oam.getOamMep().getMel()+"");
			this.localField.setText(oam.getOamMep().getLocalMepId()+"");
			this.remoteField.setText(oam.getOamMep().getRemoteMepId()+"");
			boxModel.addElement(new ControlKeyValue(t.getTunnelId() + "", t.getTunnelName(), t));
		} catch (Exception e) {
			ExceptionManage.dispose(e,this.getClass());
		}
	...
	private void getTestOam(Tunnel t, OamInfo oam) {
		...
		try {
			...
			if(oam.getOamMep().getSiteId() == t.getASiteId()){
				oamMepInfo.setSiteId(t.getASiteId());
				oamMepInfo.setObjId(t.getTunnelId());
			}else{
				oamMepInfo.setSiteId(t.getZSiteId());
				oamMepInfo.setObjId(t.getTunnelId());
			}
...
//		setAttribute();
	...
	private void setOamInfoLayout() {
		...
		componentLayout.rowHeights = new int[] { 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10 };
...
//		TunnelService sevice = null;
//		Tunnel tunnel = null;
//		List<Tunnel> list = null;
//		try {
//			sevice = (TunnelService) ConstantUtil.serviceFactory.newService(Services.Tunnel);
//			tunnel = new Tunnel();
...
//		oamMep.setMegUmc("789ABC");
		...
		AddOperateLog.insertOperLog(this.confirm, EOperationLogType.TESTOAMTUNNELUPDATE.getValue(), result, 
				oammepInfoBefore, oamInfo.getOamMep(), oamInfo.getOamMep().getSiteId(), oamInfo.getOamMep().getObjNameLog(), "testTunnelOAM");
		return result;
	}
...
//	}
	...
	public TunnelOamBusinessController getController() {
		return controller;
	}

	public void setController(TunnelOamBusinessController controller) {
