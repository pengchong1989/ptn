...
import com.nms.ui.ptn.business.testoam.dialog.SectionOamBusinessDialog;

public class SectionOamBusinessController {

	private SectionOamBusinessDialog view;

	public SectionOamBusinessController(SectionOamBusinessDialog view) {
		this.view = view;
		addListener();
	}
	...
	public boolean checkLbTTL() {
		...
		if ("".equals(view.getLbTTLField().getText()) || value > 255 || value < 1) {
			...
			view.getMainDialog().getBtnSave().setEnabled(false);
			view.getMainDialog().getBtnCanel().setEnabled(false);
			return true;
		}
		view.getVertifyLabel().setText("    ");
		view.getLbTTLField().setBorder(new LineBorder(Color.gray));
		view.getMainDialog().getBtnSave().setEnabled(true);
		view.getMainDialog().getBtnCanel().setEnabled(true);
		return false;
	}
