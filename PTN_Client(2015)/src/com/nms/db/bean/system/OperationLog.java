...
public class OperationLog extends ViewDataObj {
	...
	public void putObjectProperty() {
			...
			getClientProperties().put("operationObjName", this.getObjName());
			getClientProperties().put("siteName", this.getsiteName());
	...
	private String getObjName() {
		try {
			return operationObjName.substring(0, operationObjName.lastIndexOf(","));
		} catch (Exception e) {
			return operationObjName;
		}
	}
	...
	private String getsiteName() {
		...
		try {
			...
			if(siteId == 0){
				return ResourceUtil.srcStr(StringKeysObj.SYSTEM_LOCK);
			}else if(siteId == -1){
				return ResourceUtil.srcStr(StringKeysObj.OBJ_ALLSITE_IN_CONFIG);
			}
			else{
