...
public class SystemClockPanelCX extends JPanel{

	private static final long serialVersionUID = -2287229340607621257L;
	...
	private void init()throws Exception{
		...
		try{
			...
			this.setBorder(BorderFactory.createTitledBorder(ResourceUtil.srcStr(StringKeysPanel.PANEL_SYSTEM_CLOCK)));
			this.add(workingStatus);
			this.add(actualReferenceSource);
			this.add(workingStatus_);
			this.add(actualReferenceSource_);
			this.setGridBagLayout();/*状态->系统时钟页面布局*/
	...
	private void setGridBagLayout() throws Exception {
		...
		try {
			...
			gridBagLayout.columnWidths = new int[] { 50, 330 };
			...
			gridBagConstraints.insets = new Insets(5, 90, 5, 0);
			gridBagConstraints.fill = GridBagConstraints.BOTH;
