...
public class BfdInfo extends ViewDataObj {
	...
	public void putObjectProperty() {
		...
		try {
			...
			this.putClientProperty("bfdEnabel", UiUtil.getCodeByValue("bfdEnabel",this.getBfdEnabel()+"").getCodeName());
			this.putClientProperty("bfdMessageSendType",UiUtil.getCodeByValue("bfdMessageSendType", this.getBfdMessageSendType()+"").getCodeName());
			this.putClientProperty("mySid", this.getMySid());
			this.putClientProperty("peerSid", this.getPeerSid());
			this.putClientProperty("bfdId", this.getBfdId());
			this.putClientProperty("lspId",this.getLspId());
			this.putClientProperty("pwId",this.getPwId());
			this.putClientProperty("lspName", getLspNameByLspId(this.getLspId(),this.getSiteId()));
			this.putClientProperty("pwName", getPwNameByPwServiceId(this.getPwId(),this.getSiteId()));
		} catch (Exception e) {
	...
	private String getLspNameByLspId(int lspServiceId,int siteId) {
		TunnelService_MB tunnelService = null;
		try {
			tunnelService = (TunnelService_MB) ConstantUtil.serviceFactory.newService_MB(Services.Tunnel);
			return tunnelService.selectBySiteIdAndServiceId(siteId, lspServiceId).getTunnelName();
		} catch (Exception e) {
			ExceptionManage.dispose(e, this.getClass());
		}
		finally
	...
	private String getPwNameByPwServiceId(int pwServiceId,int siteId) {
		...
		try {
			if(pwServiceId ==0){
			   ...
			   return pwService.selectBysiteIdAndServiceId(siteId, pwServiceId).get(0).getPwName();
			}
		} catch (Exception e) {
			ExceptionManage.dispose(e, this.getClass());
		}
