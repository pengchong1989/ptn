	...
	public void putObjectProperty() {
		...
		try {
			...
			if (getOamMep() != null) {
				...
				getClientProperties().put("csfEnable", getOamMep().isCsfEnable() ? Boolean.TRUE : Boolean.FALSE);
				getClientProperties().put("oamEnable", getOamMep().isOamEnable() ? Boolean.TRUE : Boolean.FALSE);
	...
	private Object getoamName(String objectType, int objectId) throws Exception {
		...
		try {
			if ((EServiceType.SECTION.toString()+"_TEST").equals(objectType)) { 	//	段的  返回端口名
				...
				Tunnel tunnel = tunnelService.selectByID(objectId);
				if (null != tunnel ) {
					if("0".equals(tunnel.getTunnelType())){
						...
						if("2".equals(UiUtil.getCodeById(Integer.parseInt(tunnel.getTunnelType())).getCodeValue())){
							return tunnel.getTunnelName()+"_work";
						}else{
							return tunnel.getTunnelName();
						}
					}
	...
	private String getObjectName(String objectType, int objectId, int serviceId) throws Exception {
		...
		try {
			if ((EServiceType.SECTION.toString()).equals(objectType)) { // 段的 返回端口名
				...
				tunnelService = (TunnelService_MB) ConstantUtil.serviceFactory.newService_MB(Services.Tunnel);
				Tunnel tunnel  = tunnelService.selectByID(serviceId);
				if (null != tunnel) {
					if ("0".equals(tunnel.getTunnelType())) {
						return tunnel.getTunnelName();
					} else {
