 ...
 *
 */
public class WHRoatePanel extends JPanel{
	 ...
	 *   pw   对象
	 */
	public WHRoatePanel(DualInfo dualInfo,boolean flag) {
	...
	public void setRainValue(){
		...
		int roate=0;
		try{
			siteRoateServiceMB = (SiteRoateService_MB) ConstantUtil.serviceFactory.newService_MB(Services.SITEROATE);
			siteRoate = new SiteRoate();
			siteRoate.setSiteId(dualInfo.getPwProtect().getSiteId());
			siteRoate.setTypeId(this.dualInfo.getPwProtect().getId());
			siteRoate.setType("pw");
			roata = siteRoateServiceMB.select(siteRoate);
			/**
			 * 通过 pwProtect id，网元id,查找pw倒换对象
			 *    确认 此pw倒换是否有倒换值
			 *     有，则修改界面倒换命令的显示
			 */
			if(roata!=null){
				if(roata!=null){
					...
					if (roate == ERotateType.FORCESWORK.getValue()) {
						...
						lockMain.setEnabled(false);
						manpowerStand.setEnabled(false);
						forceStand.setEnabled(false);
						recoverMain.setEnabled(false);
