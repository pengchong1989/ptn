...
import com.nms.ui.ptn.ne.ces.controller.CesController;
import com.nms.ui.ptn.safety.roleManage.RootFactory;

public class CesPanel extends ContentView<CesInfo> {

	/**
	 * 
	 */
	private static final long serialVersionUID = 542884375883648683L;
	private JSplitPane splitPane;
	private JTabbedPane tabbedPane;
	private CesInfoPanel cesInfoPanel;
	// 激活右键菜单
	private PtnMenuItem activeItem;
	// 去激活右键菜单
	private PtnMenuItem unActiveItem;
	public CesPanel() {
		super("cesNodeTable",RootFactory.CORE_MANAGE);
		try {
			this.init();
	...
	private void setActionListention() {
		...
		activeItem.addActionListener(new MyActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent arg0) {
				((CesController) getController()).doActive();
			}
		...
		unActiveItem.addActionListener(new MyActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent arg0) {
				((CesController) getController()).doUnActive();
			}
	...
	private void setTabbedPaneLayout() {
		tabbedPane.add(ResourceUtil.srcStr(StringKeysTab.TAB_BASIC_INFO), this.cesInfoPanel);
	}
	...
	public void setController() {
		super.controller = new CesController(this);
	}
	...
	public CesInfoPanel getCesInfoPanel() {
		return cesInfoPanel;
	}
