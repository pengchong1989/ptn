...
public class QosSectionConfigDialog extends QosCommonConfig {
	...
	public void initComponents() {
		...
		addComponent(mainPanel, titlePanel, 0, 0, 0.0, 0.0, 1, 1,
				GridBagConstraints.BOTH, new Insets(0, 5, 5, 5),
				GridBagConstraints.NORTH, gbc);
		titlePanel.setBorder(BorderFactory.createEtchedBorder());

		addComponent(mainPanel, sectionAPanel, 0, 1, 0.1, 0.1, 1, 1,
				GridBagConstraints.BOTH, new Insets(0, 5, 5, 5),
				GridBagConstraints.SOUTH, gbc);
		addComponent(mainPanel, sectionZPanel, 0, 2, 0.1, 0.1, 1, 1,
				GridBagConstraints.BOTH, new Insets(0, 5, 5, 5),
				GridBagConstraints.SOUTH, gbc);
		addComponent(mainPanel, buttonPanel, 0, 3, 0.1, 0.0, 1, 1,
				GridBagConstraints.BOTH, new Insets(0, 5, 5, 5),
				GridBagConstraints.SOUTH, gbc);

		sectionAPanel.setLayout(new GridBagLayout());
		addComponent(sectionAPanel, namePanel, 0, 0, 0.1, 0.0, 1, 1,
				GridBagConstraints.BOTH, new Insets(0, 5, 5, 5),
				GridBagConstraints.NORTHWEST, gbc);

		addComponent(sectionAPanel, sectionAScrollPane, 0, 1, 0.1, 0.1, 1, 1,
				GridBagConstraints.BOTH, new Insets(0, 5, 5, 5),
				GridBagConstraints.NORTHWEST, gbc);

		sectionZPanel.setLayout(new GridBagLayout());
		addComponent(sectionZPanel, nameZPanel, 0, 0, 0.1, 0.0, 1, 1,
				GridBagConstraints.BOTH, new Insets(0, 5, 5, 5),
				GridBagConstraints.NORTHWEST, gbc);
		addComponent(sectionZPanel, sectionZScrollPane, 0, 1, 0.1, 0.1, 1, 1,
				GridBagConstraints.BOTH, new Insets(0, 5, 5, 5),
		...
		this.add(mainPanel);
		this.configSpinner(sectionAQosTable);
		this.configSpinner(sectionZQosTable);
	}
	...
	public void setTableModel() {
		...
		sectionAQosTableModel = new DefaultTableModel(null, columnName
				.toArray()) {
			@Override
		...
		sectionZQosTableModel = new DefaultTableModel(null, columnName
				.toArray()) {
			@Override
			public boolean isCellEditable(int row, int column) {
				if (column == 0 || column == 1 || column == 11) {
