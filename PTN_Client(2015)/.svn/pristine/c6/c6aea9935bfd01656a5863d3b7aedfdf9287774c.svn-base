...
import com.nms.ui.ptn.business.testoam.dialog.TunnelOamBusinessDialog;

public class TunnelOamBusinessController {
	private TunnelOamBusinessDialog view;

	public TunnelOamBusinessController(TunnelOamBusinessDialog view) {
		this.view = view;
		addListener();
	}
	...
	private void addListener() {
		...
		view.getLttField().addMouseListener(new MouseAdapter() {
			@Override
			public void mouseExited(MouseEvent me) {
				if (checkLtTTL()) {
					return;
		...
		view.getLttField().addKeyListener(new KeyAdapter() {
			@Override
			public void keyReleased(KeyEvent arg0) {
				if (checkLtTTL()) {
					return;
	 ...
	 * @return
	 */
	public boolean checkLtTTL(){
		int value = -1;
		try {
			value = Integer.valueOf(view.getLttField().getText());
		} catch (Exception e) {
			ExceptionManage.dispose(e, this.getClass());
		}
		if ("".equals(view.getLttField().getText()) || value > 255 || value < 1) {
			view.getVertifyLabel().setText(ResourceUtil.srcStr(StringKeysTip.TIP_LT_TTL));
			view.getLttField().setBorder(new LineBorder(Color.RED));
			view.getConfirm().setEnabled(false);
			view.getCancel().setEnabled(false);
			return true;
		}
		view.getVertifyLabel().setText("    ");
		view.getLttField().setBorder(new LineBorder(Color.gray));
		view.getConfirm().setEnabled(true);
		view.getCancel().setEnabled(true);
		return false;
	}
	...
	public boolean checkTSTTLVLength() {
		...
		if ("".equals(view.getTstTLVLengthField().getText()) || value > 65535 || value < 25) {
			view.getVertifyLabel().setText(ResourceUtil.srcStr(StringKeysTip.TIP_LIMIT_24_65536));
