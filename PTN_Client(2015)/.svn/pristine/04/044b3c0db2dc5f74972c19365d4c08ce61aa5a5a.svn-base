...
public class DualProtectRotateDialog extends PtnDialog {
	 ...
	 *            msp面板
	 */
	public DualProtectRotateDialog(DualProtect dualProtect, DualProtectPanel dualProtectPanel) {
		try {
	...
	private void btnSaveListener() {
		...
		try {
			...
			if (null != radioButton) {
				...
				this.dualProtect.setRotateOrder(rotateOrder);
				...
				if(result.equals(ResourceUtil.srcStr(StringKeysTip.TIP_CONFIG_SUCCESS))){
					...
					siteRoate.setTypeId(this.dualProtect.getId());
					siteRoate.setSiteId(this.dualProtect.getSiteId());
	...
	private void initComponent() { 
		cxPanel=new CXRoatePanel(this.dualProtect, true);
